  ...
  private action(name: TileKey, entity: Entity) {
    ...
    switch (name) {
      case "attack":
        this.getGame()
          ...
          .map((other) => {
            ...
            const diff =
              entity.getValue("strength") - other.getValue("defense");
            const clampDiff = Math.min(Math.max(diff, -10), 10) / 5;

            const multiplier =
              clampDiff < 0 ? 1 / -clampDiff : 1 + clampDiff / 2;
            const delta = Math.round(length * multiplier);

            other.setValue(
              "health",
              Math.min(other.getValue("health") - delta, MAX_HEALTH)
            );
          });
      ...
      case "heart":
        entity.setValue(
          "health",
          Math.min(entity.getValue("health") + line.length, MAX_HEALTH)
        );
        break;
    }
  }
