...
import { PathAI } from "../engine/ai/path";
import { Entity } from "../engine/entities";

export class FightAI extends PathAI {
    protected lineFromTileKeyLongestPath(entity: Entity, map: Record<number, Phaser.Math.Vector2[]>): Phaser.Math.Vector2[] {
        ...
        const otherStrength = others.reduce((prev, other) => prev + other.getValue("strength"), 0) / others.length;
        const otherDefense = others.reduce((prev, other) => prev + other.getValue("defense"), 0) / others.length;

        const lengths = Object.fromEntries(TILE_KEYS.map((key, i) => [key, map[i]?.length ?? 0] as const)) as Record<TileKey, number>;
