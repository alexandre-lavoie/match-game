...
export class PathAI extends AI {
    protected lineFromTileKeyLongestPath(_entity: Entity, tileLongestPath: Record<number, Phaser.Math.Vector2[]>): Phaser.Math.Vector2[] {
        return Object.values(tileLongestPath).reduce((prev, next) => next.length > prev.length ? next : prev, []);
    }
    ...
    public nextLine(entity: Entity): Phaser.Types.Math.Vector2Like[] {
        ...
        for (let x = 0; x < gridSize.x; x++) {
            for (let y = 0; y < gridSize.y; y++) {
                ...
                if (path.length > (tileLongestPath[tile.key]?.length ?? 0)) tileLongestPath[tile.key] = path;
            }
        }
    ...
    private searchPath(entity: Entity, x: number, y: number): Phaser.Math.Vector2[] {
        ...
        while (queue.length > 0) {
            ...
            for (let i = -1; i <= 1; i++) {
                for (let j = -1; j <= 1; j++) {
                    ...
                    if (path.some((p) => p.x === nextPoint.x && p.y === nextPoint.y)) continue;
