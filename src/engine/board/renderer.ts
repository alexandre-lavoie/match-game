...
export class BoardRenderer extends Phaser.GameObjects.Container {
    ...
    public getWorldToGrid(x: number, y: number): Phaser.Math.Vector2 | null {
        ...
        return new Phaser.Math.Vector2(Math.floor(boardPoint.x / tileSize.x), Math.floor(boardPoint.y / tileSize.y));
    }
    ...
    public onCollect(callback: (x: number, y: number, offset: number) => void, context?: EmitterContext): this {
    ...
    protected makeTileSprites(): BoardTileSprite[][] {
        return this.board.getTiles().map((col, _x) => col.map((tile, _y) => this.makeTileSprite(tile)));
    }
