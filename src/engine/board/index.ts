...
export class Board {
    protected readonly eventEmitter: Phaser.Events.EventEmitter = new Phaser.Events.EventEmitter();
    ...
    public onSelect(callback: (x: number, y: number, offset: number) => void, context?: EmitterContext): this {
    ...
    public match(points: Phaser.Types.Math.Vector2Like[]): void {
        ...
        this.eventEmitter.emit("match", points.map(p => [p?.x ?? -1, p?.y ?? -1]));
    }

    public onMatch(callback: (points: [number, number][]) => void, context?: EmitterContext): this {
        return this.callbackWrap("match", callback, context);
    }

    public pullUp(): void {
        const indices = this.tiles.map((col) => col
    ...
    public onPull(callback: (moves: [number, number, number, number][]) => void, context?: EmitterContext): this {
    ...
    public onClearLine(callback: (points: [number, number][]) => void, context?: EmitterContext): this {
    ...
    public onFill(callback: (points: [number, number][]) => void, context: EmitterContext): this {
        return this.callbackWrap("fill", callback, context);
    }

    private callbackWrap<T extends (...args: any[]) => void>(key: string, callback: T, context?: EmitterContext): this {
