...
export class EntityLineRenderer extends Phaser.GameObjects.Container {
    ...
    public constructor(scene: Phaser.Scene, entity: Entity, stroke: number, fill: number, fontSize: number) {
        super(scene);
    ...
    protected drawCount(x: number, y: number) {
        ...
        const text = new Phaser.GameObjects.Text(this.scene, x, y, this.line.getPoints().length.toString(), {
            color: `#${colorHex}`,
    ...
    protected pushPoint(x: number, y: number) {
        ...
        this.drawCount(worldPosition.x, worldPosition.y - boardRenderer.getTileSize().y / 2);
    }
